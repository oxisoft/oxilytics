<script>
  ...
  let { open = $bindable(false), title = 'Are you sure?', message = '', confirmLabel = 'Confirm', danger = false, typeToConfirm = '', onconfirm = () => {} } = $props();
  let typed = $state('');
  ...
  async function go() {
    busy = true;
    try { await onconfirm(); open = false; typed = ''; } finally { busy = false; }
  }
</script>

<Modal bind:open {title} onclose={() => (typed = '')}>
